...
cycle_number=data.variables['CYCLE_NUMBER'][:]
#basemap setup            
...
df=pd.DataFrame({'date':dates,'float_id':float_id,'latitude':lats,'longitude':lons,'sea temperature in deg_celcius':temp,'sea_water_salinity in psu':salinity,'Sea Pressure in decibar':pressure,'cycle number':cycle_number})
df.to_csv('lakshadweep_data_oct_nov17.csv')
plt.show()